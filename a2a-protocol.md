...
| Verzija | Datum | Sta |
|---|---|---|
| **v1.18.0** | 2026-05-31 | **Chat-as-peer from web UI.** New `clade_cli/web_sender.py` module: load any peer's yaml (bearer + pair-secret), build HMAC-signed envelope (reuses `agent.wire.sign`), POST to relay `/send` (fire-and-forget) or `/ask` (sync, verify reply HMAC). New `POST /api/setup/{token}/chat/send` endpoint body `{from_peer, to_peer, content, expect_reply?, timeout_s?, thread_id?}`. New "Chat" tab in result.html with from/to dropdowns, content textarea, expect-reply checkbox, optional thread_id input, and in-session recent-activity log. When `expect_reply=True`, server blocks up to `timeout_s` (default 90s) and returns the recipient's reply payload after HMAC verification; server-generated `thread_id` is surfaced back to the UI so the user can continue the thread by re-sending with that id. HMAC sign/verify extracted from `agent/main.py` into pure `agent/wire.py` so non-agent code paths (web sender) can import without triggering agent's module-level config load. Useful for: CEO/operator workflows where a human acts as a peer from the browser; smoke-testing peers without `chat-<peer>.sh`; quick directive injection into AITF setups. Validated 2026-05-31: PD → DD ask "u kom si folderu?" returned `/tmp/.../dd-daemon` workdir from real `claude --print` spawn on DD daemon. |
| **v1.17.0** | 2026-05-31 | **Web UI dashboard.** Result page (`/setup/{token}`) gets a tab navigation (Overview / Peers / Teams / Chat / Install) with localStorage + URL-hash persistence. Overview is the default "running" view: Project info + new "Cycle activity" card (per-peer scribe state table with last_head_sha / rounds_today / interval + Pause/Resume buttons; live "Recent commits" panel polling `git log` of docs_repo) + existing Live status. Setup-time UI (Add peer, Install-all, etc.) moves into Peers/Teams/Install tabs and is hidden during normal operations. Per-peer Edit form gains a "Scribe self-driving loop" fieldset (enabled / interval_minutes / max_rounds_per_day / docs_repo_path / round_prompt textarea). New backend endpoints: `POST /api/setup/{token}/pause` and `/resume` (body `{peer_id?}`) write/remove sentinel files in `$CLADE_PAUSE_DIR` (same dir the `clade-pause` CLI uses, so UI and CLI cooperate); `GET /api/setup/{token}/scribe-state` returns per-peer scribe block + persisted state file + sentinel status; `GET /api/setup/{token}/repo-log?limit=N` returns recent commits from any peer's `docs_repo_path` or `extra_add_dirs[0]`. `PATCH /api/setup/{token}/peers/{peer_id}` accepts a new `scribe: dict \| None` field — empty dict removes the scribe block, set keys merge into existing config. |
| v1.16.0 | 2026-05-31 | **Scribe kill-switch — Phase D.** Two new CLI entry points `clade-pause` and `clade-resume`. They manage sentinel files in `$CLADE_PAUSE_DIR` (default `~/.clade/`): `pause` (no args) creates `pause` → global kill-switch for ALL peers on this host; `pause --peer ID` creates `<ID>-pause` → only that peer paused. `_scribe_tick` checks for sentinels at the top of every tick (before git or spawn calls) — paused tick cheap-exits in microseconds. State transition logged once (`scribe: paused (...)` / `scribe: resumed`); not spammed per tick. Per-peer file takes priority over global in the reason string. CLI is idempotent (re-running `pause` or `resume` is safe). Does NOT terminate in-flight `claude --print` spawns — only prevents the next tick from starting one; use SIGTERM on the daemon if an immediate kill is needed. Useful when a daemon is misbehaving, when you need to land a hand-edit without scribe racing, or when API spend needs an emergency stop. Zero plumbing changes to relay or setup-server. |
| v1.15.0 | 2026-05-31 | **Orchestrator replacement — Phase C.** `ScribeConfig` gains an optional `round_prompt: str` field. When set, `_scribe_tick` uses it as the round prompt instead of the default Phase B documentation-curator instructions; context (repo path, git log since last round, peer list) is still prepended automatically. AITF import (`POST /api/setup/import-aitf`) now wires `scribe:` on **all four** roles, not just `doc`: `pd` (30min, prompt: react to verdicted REPORTS, update PROJECT_STATUS, issue next directive), `dd` (15min, prompt: break down new DIRECTIVES into TODO + verdict new REPORTS), `team` (30min, prompt: pick first unchecked TODO in current phase + implement + write report), `doc` keeps the Phase B default (60min). Each role's round is still gated by the cheap "git HEAD unchanged → return" check — peers wake only when something committed. Roles instructed to do NOTHING when the new commits aren't theirs to react to (silence is the correct answer for "no work this round"). Effectively replaces AITF's human-as-dispatcher `./start_role.sh <role>` loop and `scripts/orchestrator.sh` with self-driving daemons; the human still owns scope (writes CEO-level intent into PD's inbox or `DIRECTIVES/`). |
| v1.14.0 | 2026-05-31 | **Self-driving scribe — Phase B.** New optional `scribe:` block in peer yaml (validated as `ScribeConfig` in `agent/main.py`): `{enabled, interval_minutes (5–1440), docs_repo_path?, max_rounds_per_day (1–288)}`. When set, `agent/daemon.py` runs a fifth side-loop (`scribe_loop`) alongside `poll_loop` / `outbox_monitor_loop` / `presence_loop` / `config_watcher_loop`. Each tick is cheap: compare `git rev-parse HEAD` of the watched repo against persisted state at `<audit_db_parent>/<peer>-scribe-state.json`; if unchanged → return without spawning Claude (zero token spend on idle days). On a new HEAD, assemble a round prompt with `git log <last>..HEAD` and call a leaner `_call_claude_scribe` helper (no peer-to-peer scaffolding); Claude itself decides what to do via MCP tools (update PROJECT_STATUS / OPTIMIZATION_LOG, nudge quiet peers via `clade_message`, request CEO/PD review). State persists across daemon restarts. Hard `max_rounds_per_day` cap prevents runaway. AITF import (`POST /api/setup/import-aitf`) auto-enables scribe on the `doc` peer with sensible defaults (60min, 24/day, watches the imported project root). Other peers get scribe only if user hand-edits their yaml. Replaces AITF's on-demand `./start_role.sh doc` invocation with a self-driving loop. |
| v1.13.0 | 2026-05-31 | **AI Team Framework (AITF) import — Phase A.** New `POST /api/setup/import-aitf` endpoint takes `{project_path}` of a project scaffolded by https://github.com/dusankrstic-cpu/ai-team-framework (via its 21-question wizard) and materializes 3–4 Clade A2A peers: PD → `pd`, DD → `dd`, Team → `team`, DO → `doc` (only if `doc_optimizer_enabled: true` in `.ai-team-config.yml`). Role bodies are read verbatim from `docs/TEAM/*.md` and inlined into each peer's yaml `role` field. Every peer gets `extra_add_dirs: [<abs_project_path>]` so the daemon-spawned Claude can read AND write the AITF document substrate (`DIRECTIVES/`, `REPORTS/`, `TODO.md`, `DECISIONS.md`, `ARCHIVE/`). Teams auto-created: `aitf_team` (all four) and `engineering` (`dd` + `team`). New `clade_cli.aitf_import` module exposes `detect_aitf_project()` and `parse_aitf_project()` for programmatic use. Index page has an "Import AI Team Framework project" card that POSTs to the endpoint and redirects to the standard result page. Phase B (scribe daemon side-loop for the DO role) and Phase C (orchestrator replacement) tracked separately. |
