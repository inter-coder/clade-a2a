...
| Verzija | Datum | Sta |
|---|---|---|
| **v2.2.0** | 2026-05-17 | **Cross-host preko relay-a.** `clade/relay_poller.py` — async loop koji GET-uje `<relay_url>/inbox/<self_id>` svake 2s za peer-ove sa `self.transport=relay`. Za svaku poruku: HMAC verify sa pairwise `secret_hex`, forward u `_on_peer_envelope`, reply ide POST `/reply` (HMAC-signed). `clade/hmac.py` portovan iz v1 canonical-payload sign/verify. `HttpRemoteTransport` wired u `Server._deliver_via_transport` za relay path. `mcp_server.clade_message` signe envelope kad ide kroz relay (filesystem perms ne vaze za cross-host). `peers_config.load()` resolve-uje relativne path-ove protiv peers.yaml dir-a (portable bundles). Wizard novo `multi` mode: generise `scp`-eable per-peer bundle (`alice-bundle/`, `bob-bundle/`) + `relay-bundle/` sa `tokens.json` + INSTRUCTIONS.md za multi-machine deploy. Plus port auto-detect bug fix (proverava sve N portova slobodne). **E2E verifikovano:** alice → relay → bob → claude --print → "63" za 3.8s. |
| v2.1.0 | 2026-05-17 | Ask handler popunjava §7.1 gap iz v2.0. `clade/ask_handler.py` portuje v1 `agent/daemon.py:call_claude` logiku u `clade serve` proces: `extract_question` fallback chain, `format_thread_for_prompt` za thread continuity, `spawn_claude_print` sa minimal headless env. `Server._on_peer_envelope` za `kind="ask"` sad spawn-uje pravi `claude --print` umesto placeholder `_ack` reply-a. **Workdir je sada obavezan** za peer-ove sa `role: interactive/both` — `clade init` ga vec generise; eksplicitan error reply ako fali. **E2E verifikovano (2026-05-17, 20:06-20:07):** 4 ask roundtrip-a alice ↔ bob preko pravog `claude --print` binary-ja, 3-5s po pozivu. Test 1: aritmetika ("Koliko je 7 puta 8?" → "56"). Test 2: follow-up u istom threadu ("A koliko 12 puta 13?" → "156"). Test 3: free-form ("glavni grad Srbije" → "Beograd, ~1.4M stanovnika"). Test 4: thread context probe ("Sta sam te vec pitao?") — bob's claude navodi tacno prethodna 2 pitanja **i odgovore**, sto dokazuje da ThreadCache + format_thread_for_prompt zaista fid-uju istoriju u system prompt. Audit DB: 8 zapisa na bob strani (4 ask in + 4 reply out, svi sa thread_id-em), graceful shutdown soketi obrisani. |
